...
import 'package:custom_lint_news_api/client.dart';
import 'package:http/http.dart' as http;

/// {@template custom_lint_news_api_malformed_response}
/// An exception thrown when there is a problem decoded the response body.
/// {@endtemplate}
class CustomLintNewsApiMalformedResponse implements Exception {
  /// {@macro custom_lint_news_api_malformed_response}
  const CustomLintNewsApiMalformedResponse({required this.error});

  /// The associated error.
  final Object error;
}

/// {@template custom_lint_news_api_request_failure}
/// An exception thrown when an http request failure occurs.
/// {@endtemplate}
class CustomLintNewsApiRequestFailure implements Exception {
  /// {@macro custom_lint_news_api_request_failure}
  const CustomLintNewsApiRequestFailure({
...
/// {@template custom_lint_news_api_client}
/// A Dart API client for the CustomLintNews API.
/// {@endtemplate}
class CustomLintNewsApiClient {
  /// Create an instance of [CustomLintNewsApiClient] that integrates
  /// with the remote API.
  ///
  /// {@macro custom_lint_news_api_client}
  CustomLintNewsApiClient({
  ...
  ///
  /// {@macro custom_lint_news_api_client}
  CustomLintNewsApiClient.localhost({
    required TokenProvider tokenProvider,
    http.Client? httpClient,
  }) : this._(
  ...
  Future<ArticleResponse> getArticle({
    ...
    if (response.statusCode != HttpStatus.ok) {
      throw CustomLintNewsApiRequestFailure(
        body: body,
  ...
  Map<String, dynamic> json() {
    try {
      ...
      Error.throwWithStackTrace(
        CustomLintNewsApiMalformedResponse(error: error),
        stackTrace,
      );
    }
  }
